package com.kekadoc.tools.android.view.dialog;
...
import com.kekadoc.dialogcontextmenu.R;
...
public class DialogPopupList extends DialogFragment {
    ...
    public static DialogPopupList instance(@Size(min = 1) @NonNull Item...items) {
        if (items.length == 0) {
            throw new RuntimeException("The number of elements must not be equal to zero!");
        }
        DialogPopupList dialog = new DialogPopupList();
        dialog.items.addAll(Arrays.asList(items));
        return dialog;
    }
    public static DialogPopupList instance(View view, @Size(min = 1) @NonNull Item...items) {
        if (items.length == 0) {
            throw new RuntimeException("The number of elements must not be equal to zero!");
        }
        DialogPopupList dialog = new DialogPopupList();
        ...
        for (final Item item : items) {
            ...
            textView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    item.onSelect();
                    DialogPopupList.this.onSelectItemListener.onSelectItem(linear.indexOfChild(v));
                    dismiss();
                }
            });
    ...
    /** Build TextView */
    private TextView getTextItem(CharSequence text) {
    ...
    public interface Item {
        ...
        abstract class Simple implements Item {

            private final CharSequence text;

            public Simple(CharSequence text) {
                this.text = text;
            }
